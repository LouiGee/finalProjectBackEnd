...
/*
        if (request.getRequestURI().startsWith("/api/po/")) {
            // Skip authentication checks here, since permitAll() applies
...
*/
        ...
        // 5 .Is there a valid session associated with the token, if so create an authtoken to validate entry into API
