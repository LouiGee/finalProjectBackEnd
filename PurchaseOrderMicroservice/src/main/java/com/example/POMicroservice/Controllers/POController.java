...
public class POController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<PO>> getAll(@CookieValue(name = "authenticationToken", required = false) String authenticationToken) {
    ...
    @GetMapping("/allPaidPo")
    public ResponseEntity<List<PO>> getAllPaidPo() {
        List<PO> po = poService.getAllPaidPO();
        return new ResponseEntity<>(po, HttpStatus.OK);
    }

    @PostMapping("/approve")
    public HttpStatus approve(@RequestBody List<POItemNumber> poItemNumberList, @CookieValue(name = "email", required = false) String email) {
        poService.approvePOs(poItemNumberList, email);
        return HttpStatus.OK;
    }

    @PostMapping("/pay")
    public HttpStatus pay(@RequestBody List<POItemNumber> poItemNumberList, @CookieValue(name = "email", required = false) String email) {
        poService.payPOs(poItemNumberList, email);
        return HttpStatus.OK;
    }

    @GetMapping("/summaryStatistics")
    public ResponseEntity<List<Object>> getSummaryStatistics() {
        List<Object> summaryStatistics = poService.getSummaryStatistics();
